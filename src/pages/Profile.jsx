...
import { Mail, Phone, MapPin, Recycle, Award, LogOut, Edit3, Bell, Shield, HelpCircle, Settings, ChevronRight, Leaf, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-4 md:px-6 pb-16 pt-3 sm:pt-4">
      ...
      <div className="card mb-3 sm:mb-4 stagger-2 animate-fade-up p-4 sm:p-6">
        ...
        {!user?.emailVerified ? (
          <div className="mt-4 p-4 rounded-2xl bg-yellow-50 border border-yellow-100 text-yellow-800">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="font-semibold">Email not verified</p>
                <p className="text-xs text-yellow-700 mt-1">Verify your email to receive notifications and unlock full account access.</p>
              </div>
              <button
                type="button"
                onClick={async () => {
                  ...
                  setVerificationMessage(result.success ? 'Verification email sent. Check your inbox.' : result.error || 'Unable to send verification email.');
                  setIsSendingVerification(false);
                }}
                disabled={isSendingVerification}
                className="inline-flex items-center justify-center rounded-full bg-yellow-600 px-4 py-2 text-xs font-semibold text-white hover:bg-yellow-700 disabled:opacity-70"
              >
                {isSendingVerification ? 'Sending…' : 'Resend'}
              </button>
            </div>
            {verificationMessage && <p className="mt-3 text-xs">{verificationMessage}</p>}
          </div>
        ) : null}
      </div>
