...
export default function Onboarding() {
  ...
  return (
    <div className="min-h-screen bg-eco-600 relative overflow-hidden flex flex-col">
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-eco-500/30 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3" />
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-eco-700/40 rounded-full blur-3xl translate-y-1/2 -translate-x-1/4" />
      ...
      {/* Content */}
      <div className="relative z-10 flex flex-col items-center justify-between min-h-screen px-6 py-12 max-w-lg mx-auto w-full">
        
        {/* Logo + Hero */}
        <div className="flex flex-col items-center text-center pt-8 animate-fade-up">
          <div className="w-24 h-24 bg-white/15 backdrop-blur-sm rounded-3xl flex items-center justify-center mb-6 border border-white/20 shadow-2xl animate-float p-3">
            <img src={ecoAlertLogo} alt="EcoAlert logo" className="w-full h-full object-contain" />
          </div>
          <p className="text-eco-200 text-sm font-mono tracking-widest uppercase mb-4">EcoAlert v1.0</p>
          <h1 className="text-5xl font-display font-black text-white leading-[1.1] mb-5">
            Clean<br />Communities<br />
            <span className="text-eco-300">Start With You.</span>
          </h1>
          <p className="text-eco-100/80 text-lg leading-relaxed max-w-sm">
            Report waste, track clean-ups, and join thousands making a real difference in Ghana.
          </p>
        </div>

        {/* Feature cards */}
        <div className="my-8 w-[min(100vw-3rem,56rem)] grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="w-full space-y-3">
          <button
            ...
            className="w-full bg-white text-eco-700 font-display font-bold py-4 px-8 rounded-2xl text-lg flex items-center justify-center gap-2 hover:bg-eco-50 active:scale-95 transition-all duration-200 shadow-2xl"
          >
            Get Started
            <ArrowRight size={20} />
          </button>
